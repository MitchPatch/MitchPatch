...
function installupdates
{
		...
		else
		{
			...
			if ($updatenumber -ne '0')
			{
				...
				do
				{
					...
					if ($cred -eq "")
					{
						invoke-command -ComputerName $c -ScriptBlock { Invoke-WUjob -RunNow -ComputerName localhost -Script "ipmo PSWindowsUpdate; Install-WindowsUpdate -AcceptAll -IgnoreReboot | Out-File C:\PSWindowsUpdate.log" -Confirm:$false }
					}
					else
					{
						invoke-command -ComputerName $c -Credential $cred -ScriptBlock { Invoke-WUjob -RunNow -ComputerName localhost -Script "ipmo PSWindowsUpdate; Install-WindowsUpdate -AcceptAll -IgnoreReboot | Out-File C:\PSWindowsUpdate.log" -Confirm:$false }
	...
	foreach ($c in $to_update)
	{
		...
		$PowerShell.AddScript($Scriptblock).AddArgument($c).AddArgument($cred).AddArgument($InstallOnlyKB).AddArgument($ExcludeKB).AddArgument($doRestartFinal).AddArgument($Configuration) | Out-Null
