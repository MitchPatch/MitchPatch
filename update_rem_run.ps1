function installupdates_rem
{
	$Scriptblock = {
		...
		else
		{
			...
			if ($cred -eq "")
			{
				$updates = invoke-command -ComputerName $c -scriptblock { Get-WindowsUpdate -NotKBArticleID $using:ExcludeKB -verbose }
			}
			else
			{
				$updates = invoke-command -ComputerName $c -Credential $cred -scriptblock { Get-WindowsUpdate -NotKBArticleID $using:ExcludeKB -verbose }
			}
			...
			if ($updatenumber -ne '0')
			{
				...
				do
				{
					...
					if ($cred -eq "")
					{
						invoke-command -ComputerName $c -ScriptBlock { Invoke-WUjob -RunNow -ComputerName localhost -Script "ipmo PSWindowsUpdate; Install-WindowsUpdate -NotKBArticleID $using:ExcludeKB -AcceptAll -IgnoreReboot | Out-File C:\PSWindowsUpdate.log" -Confirm:$false }
					}
					else
					{
						invoke-command -ComputerName $c -Credential $cred -ScriptBlock { Invoke-WUjob -RunNow -ComputerName localhost -Script "ipmo PSWindowsUpdate; Install-WindowsUpdate -NotKBArticleID $using:ExcludeKB -AcceptAll -IgnoreReboot | Out-File C:\PSWindowsUpdate.log" -Confirm:$false }
					}
	...
	foreach ($c in $to_update)
	{
		...
		$PowerShell.AddScript($Scriptblock).AddArgument($c).AddArgument($cred).AddArgument($ExcludeKB).AddArgument($doRestartFinal).AddArgument($Configuration) | Out-Null
